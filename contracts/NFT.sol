...
contract NFT is ERC721URIStorage {
	...
	event CatNFTMinted(address sender, uint256 tokenId, string tokenURI);
	...
	// array of structs to keep track of all our NFTs, so we can return specific metadata at will.
	struct Token {
		uint256 tokenId;
		string tokenURI;
	}
	...
	function pickFirstRandomWord(uint256 tokenId) private view returns (string memory) {
		uint256 rand = random(string(abi.encodePacked('FIRST_WORD', Strings.toString(tokenId))));
		rand = rand % sizeSet.length;
		return sizeSet[rand];
	}

	function pickSecondRandomWord(uint256 tokenId) private view returns (string memory) {
		uint256 rand = random(string(abi.encodePacked('SECOND_WORD', Strings.toString(tokenId))));
		rand = rand % colorSet.length;
		return colorSet[rand];
	}

	function pickThirdRandomWord(uint256 tokenId) private view returns (string memory) {
	...
	function mintCatNFT() public {
		...
		// emit event from smart contract to let listeners know NFT was minted
		emit CatNFTMinted(msg.sender, newItemId, finalTokenURI);
